...
const RECEPTION_NAV = [
  ...
  ["appointments", "Appointments", "Patient Flow"],
  ["patients", "Patients", "Patient Flow"],
  ["queue", "Check-In / Queue", "Patient Flow"],
  ["admissions", "Admissions", "Patient Flow"],
  ["billing", "Billing", "Front Desk"],
  ["checkout", "Checkout", "Front Desk"],
  ["followups", "Follow-ups", "Front Desk"],
  ["notifications", "Notifications", "Communication"],
  ["globalSearch", "Global Search", "Communication"],
  ["records", "My Records", "Work"],
  ["tasks", "My Tasks", "Work"],
  ["alerts", "Alerts", "Work"]
];
...
const MORTUARY_NAV=[["dashboard","My Dashboard","Overview"],["mortuary","Cases","Mortuary"],["mortuary-intake","Admissions / Intake","Mortuary"],["mortuary-storage","Body Storage","Mortuary"],["mortuary-release","Release / Handover","Mortuary"],["mortuary-search","Patient / Case Search","Records"],["documents","Documents","Records"],["tasks","My Tasks","Work"],["alerts","Alerts","Work"],["reports","Reports","Reports"]];
...
function journeyTracker(steps, activeIndex = 0, label = "Workflow") {
  const routeByStep = {
    ...
    "lab / pharmacy": hasPermission(currentUser, "lab", "view") ? "lab" : "pharmacy",
    lab: "lab",
    pharmacy: "pharmacy",
    billing: "billing",
    checkout: "checkout"
  ...
  return `
    <section class="journey-card" aria-label="${escapeHtml(label)}">
      <div class="journey-head">
        <strong>${escapeHtml(label)}</strong>
        <span>${escapeHtml(steps[Math.min(activeIndex, steps.length - 1)] || "")}</span>
      </div>
      <div class="journey-steps">
        ${steps.map((step, index) => {
          const route = routeByStep[String(step).toLowerCase()];
          const allowed = route && canAccessPage(currentUser, route);
          const attrs = allowed ? `role="button" tabindex="0" data-route="${route}"${selectedPatientId ? ` data-patient-id="${escapeHtml(selectedPatientId)}"` : ""}` : "";
          return `
            <span class="${index < activeIndex ? "done" : index === activeIndex ? "active" : ""} ${allowed ? "clickable-step" : "disabled-step"}" ${attrs}>
...
function patientTimeline(context = {}) {
  ...
  const stages = [
    ...
    ["lab", "Lab Ordered"],
    ["pharmacy", "Medicine Issued"],
    ["billing", "Bill Paid"],
    ["checkout", "Checkout Completed"]
  ];
  return `
    <section class="timeline" data-testid="patient-timeline">
      ${stages.map(([key, label]) => `<span class="${completed.has(key) ? "done" : key === current ? "active" : ""}">${escapeHtml(label)}</span>`).join("")}
    </section>
  `;
}

function opdJourneyTrackerForPatient(patientId, data = deriveOperationalData(), label = "OPD Journey Tracker") {
  const hasAppointment = data.appointments.some((item) => String(item.patientId) === String(patientId));
  const hasQueue = data.queue.some((item) => String(item.patientId) === String(patientId));
  const hasVitals = data.vitals.some((item) => String(item.patientId) === String(patientId));
  const token = [...data.queue].filter((item) => String(item.patientId) === String(patientId)).sort((a, b) => new Date(b.createdAt || b.checkedInAt || 0) - new Date(a.createdAt || a.checkedInAt || 0))[0];
  const encounterJourney = token ? opdConsultationJourney(token, { consultations: data.consultations, labOrders: data.labOrders || [], radiologyOrders: data.radiologyOrders || [], prescriptions: data.prescriptions || [] }) : null;
  const hasConsultation = encounterJourney?.consultationStage === "Completed";
  const hasBill = data.bills.some((item) => String(item.patientId) === String(patientId));
  const hasPaidBill = data.bills.some((item) => String(item.patientId) === String(patientId) && item.status === "Paid");
  const steps = [
    ["Registration", "Completed", "patients"],
    ["Appointment", hasAppointment ? "Completed" : "Pending", "appointments"],
    ["Check-in", hasQueue ? "Completed" : "Pending", "appointments"],
    ["Vitals", hasVitals ? "Completed" : "Pending", "vitals"],
    ["Consultation", encounterJourney?.consultationStage || "Pending", "consultation"],
    ["Lab / Radiology", encounterJourney?.investigationStage || "Pending", "lab"],
    ["Pharmacy", encounterJourney?.pharmacyStage || "Pending", "pharmacy"],
    ["Billing", hasBill ? "Completed" : "Pending", "billing"],
    ["Checkout", hasPaidBill ? "Completed" : "Pending", "checkout"]
  ];
  const activeIndex = steps.findIndex(([, status]) => !["Completed", "Not Required"].includes(status));
  return `
    <section class="journey-card opd-journey" aria-label="${escapeHtml(label)}">
      <div class="journey-head">
        <strong>${escapeHtml(label)}</strong>
        <span>${escapeHtml(activeIndex === -1 ? "Checkout" : steps[activeIndex][0])}</span>
      </div>
      <div class="journey-steps">
        ${steps.map(([step, stepStatus, route], index) => {
          const done = stepStatus === "Completed";
          const skipped = stepStatus === "Not Required";
          const current = index === activeIndex;
          const allowed = canAccessPage(currentUser, route);
          const blocked = step === "Billing" && hasConsultation && !hasBill && !hasPermission(currentUser, "billing", "view");
          const attrs = allowed ? `role="button" tabindex="0" data-route="${route}" data-patient-id="${escapeHtml(patientId)}"` : "";
          return `
            <span class="${done ? "done" : skipped ? "skipped" : current ? "active" : blocked ? "blocked" : ""} ${allowed ? "clickable-step" : "disabled-step"}" ${attrs}>
              <i>${index + 1}</i>${escapeHtml(step)}<small>${escapeHtml(blocked ? "Blocked" : stepStatus)}</small>
            </span>
          `;
        }).join("")}
      </div>
...
function patientJourneyTimelinePanel(patient = {}, data = deriveOperationalData()) {
  const patientId = patient.id;
  const events = [];
  const add = (when, title, module, status, user = "-", route = "patients") => {
    if (patientId && (when || title)) events.push({ when: when || patient.createdAt || new Date().toISOString(), title, module, status, user, route });
  };
  add(patient.createdAt || patient.registrationDate, "Registration", "Patients", patient.status || "Registered", patient.createdBy || patient.registeredBy);
  data.appointments.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(item.date || item.createdAt, "Appointment booking", "Appointments", item.status || "Booked", item.createdBy || item.doctor, "appointments"));
  data.queue.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), "Check-in", "Queue", item.status || "Waiting", item.createdBy || item.checkedInBy, "queue"));
  data.vitals.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(item.dateTime || item.createdAt, "Vitals recorded", "Vitals", item.status || "Recorded", item.recordedBy, "vitals"));
  data.consultations.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(item.startedAt || item.createdAt, "Consultation started", "Consultation", item.status || "Completed", item.doctor || item.createdBy, "consultation"));
  data.labOrders.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), item.status === "Report Ready" ? "Report ready" : `${item.orderType || "Lab"} ordered`, item.orderType || "Lab", item.status || "Ordered", item.doctor || item.createdBy, item.orderType === "Radiology" ? "radiology" : "lab"));
  data.pharmacyIssues.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), "Pharmacy issue", "Pharmacy", item.status || "Pending", item.issuedBy || item.createdBy, "pharmacy"));
  data.bills.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), item.status === "Paid" ? "Payment collected" : "Bill generated", "Billing", item.status || "Generated", item.createdBy, "billing"));
  data.checkouts.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), "Checkout completed", "Checkout", item.status || "Completed", item.createdBy, "checkout"));
  data.admissions.filter((item) => String(admissionPatientId(item)) === String(patientId) || String(item.patientId) === String(patientId)).forEach((item) => {
    add(item.requestedAt || item.createdAt, "Admission requested", "Admissions", item.status || "Requested", item.requestedBy || item.createdBy, "admissions");
    if (item.bedNumber || item.bedId) add(item.updatedAt || item.createdAt, "Bed assigned", "Wards", item.status || "Active", item.updatedBy || item.createdBy, "wards");
  });
  data.dischargePlans.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), "Discharge initiated", "Discharge", item.status || "Pending", item.createdBy, "discharge"));
  data.deathSummaries.filter((item) => String(item.patientId) === String(patientId) || item.mrn === patient.mrn).forEach((item) => add(recordTime(item), `Death Summary ${item.status || "updated"}`, "Death Summary", item.status || "Updated", item.updatedBy || item.createdBy, "ipdPatient360"));
  data.documents.filter((item) => String(item.patientId) === String(patientId)).forEach((item) => add(recordTime(item), "Documents uploaded", "Documents", item.status || "Uploaded", item.uploadedBy || item.createdBy, "documents"));
  const sorted = events.sort((a, b) => new Date(b.when) - new Date(a.when)).slice(0, 20);
  const [risk, riskClassName] = patientRiskIndicator(patientId, data);
  return `
    <section class="panel" data-testid="patient-journey-timeline">
      <div class="panel-head"><div><h3>Patient Journey Timeline</h3><p>Registration, OPD, IPD, billing, documents, and audit-facing milestones visible to your role.</p></div>${badge(risk, riskClassName)}</div>
      ${sorted.length ? table(["Time", "Event", "Module", "Status", "User / Role", "Branch"], sorted.map((event) => [
        formatDateTime(event.when),
        canAccessPage(currentUser, event.route) ? rowRouteButton(event.title, event.route, { patientId }) : event.title,
        event.module,
        badge(event.status, statusClass(event.status)),
        event.user || currentUser.jobRole || currentUser.role,
        patient.branchName || currentUser.branchName || currentUser.branchId || "-"
      ])) : emptyState("No journey events are visible for this patient yet.")}
      <div class="notice subtle">Timeline is generated from existing records and audit-backed actions where available.</div>
    </section>
  `;
}
...
function ipdJourneyTracker(admission, context = {}) {
  const steps = [
    ["admissions", "Admission", Boolean(admissionDisplayId(admission))],
    ["wards", "Bed Assigned", Boolean(admission.bedNumber || admission.bedId)],
    ["dailySheets", "Daily Care", Boolean(context.sheets?.length)],
    ["dutyDoctor", "Doctor Notes", Boolean(context.doctorNotes?.length)],
    ["nursing", "Nursing Notes", Boolean(context.nursingNotes?.length)],
    ["mar", "Medication", Boolean(context.mar?.length)],
    ["intakeOutput", "Intake/Output", Boolean(context.intakeOutput?.length)],
    ["handover", "Handover", Boolean(context.handovers?.length)],
    ["discharge", "Clearance", Boolean(context.dischargePlans?.length)],
    ["discharge", "Discharge", ["Discharged", "Ready for Discharge"].includes(ipdAdmissionStatus(admission))]
  ];
  const firstPending = steps.findIndex(([, , done]) => !done);
  return `
    <section class="journey-card ipd-journey" data-testid="ipd-360-journey">
      <div class="journey-head">
        <strong>IPD Journey</strong>
        <span>${escapeHtml(firstPending === -1 ? "Discharge" : steps[firstPending][1])}</span>
      </div>
      <div class="journey-steps">
        ${steps.map(([route, label, done], index) => {
          const allowed = canAccessPage(currentUser, route);
          const state = done ? "Completed" : index === firstPending ? "Current" : "Pending";
          const attrs = allowed ? `role="button" tabindex="0" data-route="${route}" data-admission-id="${escapeHtml(admissionDisplayId(admission))}"` : `title="You do not have permission to open this section."`;
          return `
            <span class="${done ? "done" : index === firstPending ? "active" : ""} ${allowed ? "clickable-step" : "disabled-step"}" ${attrs}>
              <i>${index + 1}</i>${escapeHtml(label)}<small>${escapeHtml(state)}</small>
            </span>
          `;
        }).join("")}
      </div>
...
function ipdTimelinePanel(events = []) {
  return `
    <section class="panel">
      <div class="panel-head"><h3>IPD Patient 360 Timeline</h3><p>Important events from visible admission, clinical, billing, document, and workflow records.</p></div>
      ${events.length ? `<div class="event-timeline">${events.map((event) => `
        <div class="timeline-event">
          <span>${escapeHtml(formatDateTime(event.when))}</span>
          <strong>${escapeHtml(event.title)}</strong>
          <small>${escapeHtml(event.module)}${event.user ? ` / ${escapeHtml(event.user)}` : ""}</small>
        </div>
      `).join("")}</div>` : emptyState("No timeline events are visible for this admission yet.")}
    </section>
  `;
}
...
function livePatientFlowBoard(flows = [], queue = [], vitals = [], consultations = [], labOrders = [], radiologyOrders = [], prescriptions = []) {
  const withEncounterStatus = (flow) => {
    const token = queue.find((item) => String(item.id) === String(flow.queueTokenId || flow.id))
      || queue.find((item) => String(item.patientId) === String(flow.patientId) && flow.appointmentId && String(item.appointmentId) === String(flow.appointmentId))
      || [...queue].filter((item) => String(item.patientId) === String(flow.patientId)).sort((a, b) => new Date(b.updatedAt || b.createdAt || b.checkedInAt || 0) - new Date(a.updatedAt || a.createdAt || a.checkedInAt || 0))[0]
      || flow;
    const vitalsStatus = opdVitalsStage(token, vitals);
    const journey = opdConsultationJourney(token, { consultations, labOrders, radiologyOrders, prescriptions });
    return {
      ...flow,
      currentLabel: token.status || flow.currentLabel,
      steps: (flow.steps || []).map((step) => {
        if (step.key === "vitals") return { ...step, status: vitalsStatus };
        if (step.key === "consultation") return { ...step, status: journey.consultationStage };
        if (step.key === "lab") return { ...step, status: journey.investigationStage };
        if (step.key === "pharmacy") return { ...step, status: journey.pharmacyStage };
        return step;
      })
    };
  };
  const activeFlows = flows.length ? flows.map(withEncounterStatus) : queue.map((token) => {
    const journey = opdConsultationJourney(token, { consultations, labOrders, radiologyOrders, prescriptions });
    return ({
    id: token.id,
    patientId: token.patientId,
    ...
    currentLabel: token.status || "Waiting",
    currentRoute: "queue",
    status: "In Progress",
    steps: [
      { key: "checkIn", label: "Check-in", route: "queue", status: "Completed" },
      { key: "vitals", label: "Vitals", route: "vitals", status: opdVitalsStage(token, vitals) },
      { key: "consultation", label: "Consultation", route: "consultation", status: journey.consultationStage },
      { key: "lab", label: "Lab / Radiology", route: "lab", status: journey.investigationStage },
      { key: "pharmacy", label: "Pharmacy", route: "pharmacy", status: journey.pharmacyStage },
      { key: "billing", label: "Billing", route: "billing", status: "Pending" },
      { key: "checkout", label: "Checkout", route: "checkout", status: "Pending" }
    ]
  }); });
  if (!activeFlows.length) return "";
  const statusClassForStep = (status = "") => {
    if (status === "Completed") return "done";
    if (status === "In Progress") return "active";
    if (["Skipped", "Not Required"].includes(status)) return "skipped";
    return "pending";
  };
  return `
    <section class="panel live-flow-panel">
      <div class="panel-head">
        <div><h3>Live patient flow</h3><p>Each row moves automatically as staff finish the current screen task.</p></div>
        <span class="badge status-active">${activeFlows.filter((flow) => flow.status !== "Completed").length} active</span>
      </div>
      <div class="live-flow-list">
        ${activeFlows.map((flow) => `
          <article class="live-flow-card" data-route="${escapeHtml(flow.currentRoute || "queue")}" data-patient-id="${escapeHtml(flow.patientId || "")}" role="button" tabindex="0">
            <div class="live-flow-head">
              <div><strong>${escapeHtml(flow.patientName || flow.patientId || "Patient")}</strong><small>${escapeHtml(flow.tokenNumber || flow.patientId || "")}</small></div>
              ${badge(flow.currentLabel || "In Progress", flow.status === "Completed" ? "status-active" : "risk-medium")}
            </div>
            <div class="live-flow-steps">
              ${(flow.steps || []).map((step) => `
                <span class="${statusClassForStep(step.status)}">
                  <i></i>
...
document.addEventListener("click", async (event) => {
  ...
  // Create actions are handled before route and feature-specific delegation so
  // clicking either the button or an SVG/icon inside it always opens the modal.
...
// Re-render whenever background data finishes loading (non-blocking data cache).
...
// Warm the data cache in the background so screens render from cache instead of
// blocking on the network (fixes the slow/"stuck on LOADING" experience).
